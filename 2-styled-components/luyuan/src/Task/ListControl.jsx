...
const CheckAllTaskButton = ({list,handler}) => {
    if (list.length > 0) {
        return <Checkbutton type="checkbox" onClick={handler} />
    }
    return "";
}

const ListControl = ({list,handlerCheckAll,handlerComplete,handlerDel}) => {
    return (
        <Listmain>
            ...
            <Listul>
                {
                    list.map((item, index) => {
                        return (
                            <Listli key={index} status={item.iscomplete}>                  
                                    <Listlicheckbox type="checkbox" onClick={() => { handlerComplete(item) }} checked={item.iscomplete} />
                                    <Listlilable>{item.text}</Listlilable>
