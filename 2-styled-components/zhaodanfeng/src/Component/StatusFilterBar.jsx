...
class StatusFilterBar extends Component{
    ...
    handleFilter = (e) => {
        this.setState({
            checkedCode: e.currentTarget.id
        });
    ...
    render() {
        ...
        filterActions = filterActions.map(element => (
            <BtnLi key={element.code}><BtnA href="javascript:;" checked={element.code === this.state.checkedCode} id={element.code} onClick={this.handleFilter}>{element.name}</BtnA></BtnLi>
        ));
        return (
            <TodoFooter>
