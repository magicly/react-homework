...
const InputView = props => {
    return (
        <NewTodo>
            <ToggleAll type="checkbox" checked={props.checked} onChange={() => props.onChange()} />
    ...
    if (arr.data.length > 0) {
        list = arr.data.map((item, index) => {
            return (
                <CustomBox key={index}>
                    <HandleCheck>
                        <CustomIpt type="checkbox" checked={item.isChecked} onChange={() => props.checkOne(index)} ></CustomIpt>
                    </HandleCheck>
