...
class Todo extends Component {
  ...
  saveItems = data => {
    console.log("save-------------------------------------" + JSON.stringify(data || this.state.dataArr));
  ...
  getLeftItem = () => {
    let item = 0;
    let tempArr = _.clone(this.state.dataArr);
    const arr = tempArr.filter(e => !e.isChecked);
    console.log("arr---->" + arr);
    item = arr.length;
    this.setState({
      itemLeft: item
    });
    console.log(`${"this.state.dataArr :" + this.state.dataArr + "-------------item :" + item}`);
  };
